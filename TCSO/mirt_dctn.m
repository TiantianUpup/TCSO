function a = mirt_dctn(a)
    ...
    % n2=n2/3;
    % a = zeros(n1, n2, 3);
    % a(:, :, 1) = X(n1 + 1:2 * n1, 2 * n2 + 1:3 * n2);
    % a(:, :, 2) = -X(1:n1, 2 * n2 + 1:3 * n2);
    % a(:, :, 3) = X(1:n1, n2 + 1:2 * n2);
    %disp(a);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% Check input
    ...
    % Y(1:n1,n2+1:2*n2)=a(:,:,3);
    % Y(1:n1,2*n2+1:3*n2)=-a(:,:,2);
    % Y(n1+1:2*n1,1:n2)=-a(:,:,3);
    % Y(n1+1:2*n1,2*n2+1:3*n2)=a(:,:,1);
    % Y(2*n1+1:3*n1,1:n2)=a(:,:,2);
    % Y(2*n1+1:3*n1,n2+1:2*n2)=-a(:,:,1);
